...
V4.9 Supabase SQL을 먼저 실행했는지 확인하세요.`);return}
  ...
  async function addDepartment(e){e.preventDefault();const name=newDepartment.trim();if(!name||!departmentFacility)return;const r=await supabase.from('org_departments').insert({name,facility_id:departmentFacility,sort_order:departments.length+1});if(r.error)alert(r.error.message);else{setNewDepartment('');load()}}
  ...
  return <>
    ...
    {tab==='master'&&<div className="admin-grid master-data-grid"><Panel title="구분 목록"><form className="inline-add" onSubmit={addFacility}><input value={newFacility} onChange={e=>setNewFacility(e.target.value)} placeholder="새 구분명"/><button className="primary"><Plus size={16}/> 추가</button></form><div className="master-list">{facilities.map(f=><div key={f.id} className={!f.is_active?'master-inactive':''}><span><strong>{f.name}</strong><small>{f.is_active?'가입·직원등록 화면의 구분에 표시':'신규 선택 목록에서 숨김'}</small></span><button className={f.is_active?'secondary compact-button':'primary compact-button'} onClick={()=>toggleFacility(f)}>{f.is_active?'비활성화':'활성화'}</button></div>)}</div></Panel><Panel title="소속 시설 목록"><form className="inline-add stacked-add" onSubmit={addDepartment}><select required value={departmentFacility} onChange={e=>setDepartmentFacility(e.target.value)}><option value="">구분 선택</option>{facilities.filter(f=>f.is_active).map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select><input value={newDepartment} onChange={e=>setNewDepartment(e.target.value)} placeholder="새 시설명"/><button className="primary"><Plus size={16}/> 추가</button></form><div className="master-list">{departments.map(d=><div key={d.id} className={!d.is_active?'master-inactive':''}><span><strong>{d.name}</strong><small>{facilityName(d.facility_id)} · {d.is_active?'가입·직원등록 화면에 표시':'신규 선택 목록에서 숨김'}</small></span><button className={d.is_active?'secondary compact-button':'primary compact-button'} onClick={()=>toggleDepartment(d)}>{d.is_active?'비활성화':'활성화'}</button></div>)}{!departments.length&&<div className="empty compact">등록된 시설이 없습니다.</div>}</div></Panel><Panel title="직위 목록"><form className="inline-add" onSubmit={addPosition}><input value={newPosition} onChange={e=>setNewPosition(e.target.value)} placeholder="새 직위명"/><button className="primary"><Plus size={16}/> 추가</button></form><div className="master-list">{positions.map(p=><div key={p.id} className={!p.is_active?'master-inactive':''}><span><strong>{p.name}</strong><small>{p.is_active?'직원등록 화면에 표시':'신규 선택 목록에서 숨김'}</small></span><button className={p.is_active?'secondary compact-button':'primary compact-button'} onClick={()=>togglePosition(p)}>{p.is_active?'비활성화':'활성화'}</button></div>)}{!positions.length&&<div className="empty compact">등록된 직위가 없습니다.</div>}</div></Panel><Panel title="운영 안내"><div className="guide-box"><strong>기초정보 사용 방법</strong><p>활성 구분·시설·직위만 가입·직원등록 화면의 선택 목록에 표시됩니다.</p><p>기존 기록 보호를 위해 기초정보는 삭제하지 않고 비활성화합니다. 기존 직원의 소속 시설과 직위 값은 그대로 유지됩니다.</p><p>직원 등록 시 구분과 소속 시설을 선택하면 같은 시설의 활성 팀장만 담당 팀장 목록에 표시됩니다.</p></div></Panel></div>}
