...
function AuthScreen(){
  ...
  return <div className="auth-wrap">
    ...
    <div className="auth-form-side"><form className="auth-card" onSubmit={submit}>
      ...
      {mode==='signup'&&<><label>이름<input required value={name} onChange={e=>setName(e.target.value)} placeholder="홍길동"/></label><label>소속 시설<select required value={facilityId} onChange={e=>{setFacilityId(e.target.value);setDepartment('')}}><option value="">시설을 선택하세요</option>{facilities.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select></label><label>소속 부서<select required value={department} onChange={e=>setDepartment(e.target.value)} disabled={!facilityId}><option value="">부서를 선택하세요</option>{departments.filter(d=>String(d.facility_id)===String(facilityId)).map(d=><option key={d.id} value={d.name}>{d.name}</option>)}</select><small className="field-help">시설을 먼저 선택하세요. 목록에 없다면 관리자에게 등록을 요청하세요.</small></label></>}
  ...
  return <>
    ...
    <div className={`dashboard-grid ${dashboardEdit?'is-editing':''}`}>
      {dashboardOrder.map(cardId=>{
        ...
        if(cardId==='interview-count')return <div key={cardId} className="dashboard-card-shell full-width-card" {...dragProps}>{handle}<div className="panel interview-count-panel"><div className="panel-head"><div><h2>팀원별 면담 횟수</h2><p className="panel-description">이번 달 면담 현황과 누적 기록을 한눈에 확인합니다.</p></div><span className="panel-caption">재직 팀원 {interviewStats.length}명</span></div><div className="interview-count-list">{interviewStats.length?interviewStats.map(person=><button type="button" className="interview-count-row" key={person.id} onClick={()=>navigate('interviews',person.id)}><span className="interview-person"><span className="avatar small">{person.name?.[0]||'직'}</span><span><strong>{person.name}</strong><small>{person.department||'부서 미지정'}{person.position?` · ${person.position}`:''}</small></span></span><span className="interview-count-metrics"><span><small>이번 달</small><strong>{person.monthCount}회</strong></span><span><small>누적</small><strong>{person.totalCount}회</strong></span><span className="last-interview"><small>최근 면담</small><strong>{person.lastInterview?new Date(person.lastInterview).toLocaleDateString('ko-KR'):'기록 없음'}</strong></span><ArrowRight size={16}/></span></button>):<div className="empty compact">표시할 재직 팀원이 없습니다.</div>}</div></div></div>
...
function Employees({profile}){
  ...
  function open(row){setSelected(row?{...row}:{...employeeBlank,facility_id:profile.facility_id||'',manager_id:canAssign?'':profile.id})}
  async function save(e){e.preventDefault();const {id,...values}=selected;const payload={...values,facility_id:isFacilityManager?profile.facility_id:values.facility_id,manager_id:canAssign?(values.manager_id||profile.id):profile.id};const r=id?await supabase.from('employees').update(payload).eq('id',id):await supabase.from('employees').insert(payload);if(r.error)alert(r.error.message);else{setSelected(null);load()}}
  ...
  const facilityName=id=>facilities.find(x=>String(x.id)===String(id))?.name||'시설 미지정'
  return <>
    ...
    <div className="employee-grid">{filtered.map(x=><div className="employee-card" key={x.id}><div className="avatar large">{(x.name||'?')[0]}</div><div className="grow"><h3>{x.name||'이름 미입력'} <Badge status={x.employment_status}/></h3><p>{facilityName(x.facility_id)} · {x.department||'부서 미지정'} · {x.position||'직위 미지정'}</p><small>{canAssign?`담당 팀장: ${managerName(x.manager_id)}`:`입사일: ${x.joined_date||'-'}`}</small></div><div className="actions"><button className="icon-btn" onClick={()=>open(x)} title="수정"><Pencil size={16}/></button><button className="icon-btn" onClick={()=>remove(x)} title="퇴직 처리"><UserX size={16}/></button>{isAdmin&&<button className="icon-btn danger-text" onClick={()=>hardDelete(x)} title="완전 삭제"><Trash2 size={16}/></button>}</div></div>)}</div>
    ...
    {selected&&<Modal title={selected.id?'팀원 수정':'팀원 추가'} onClose={()=>setSelected(null)}><form className="form-grid" onSubmit={save}><div className="form-section-title span2">기본정보</div><label>이름<input required value={selected.name||''} onChange={e=>setSelected({...selected,name:e.target.value})}/></label><label>소속 시설<select required disabled={isFacilityManager} value={selected.facility_id||''} onChange={e=>setSelected({...selected,facility_id:e.target.value,department:'',manager_id:''})}><option value="">시설을 선택하세요</option>{facilities.filter(f=>f.is_active||String(f.id)===String(selected.facility_id)).map(f=><option key={f.id} value={f.id}>{f.name}{!f.is_active?' (비활성)':''}</option>)}</select></label><label>소속 부서<select required value={selected.department||''} onChange={e=>setSelected({...selected,department:e.target.value,manager_id:''})}><option value="">부서를 선택하세요</option>{departments.filter(d=>(d.is_active||d.name===selected.department)&&String(d.facility_id)===String(selected.facility_id)).map(d=><option key={d.id} value={d.name}>{d.name}{!d.is_active?' (비활성)':''}</option>)}</select><small className="field-help">선택한 시설에 등록된 부서만 표시됩니다.</small></label><label>직위<select required value={selected.position||''} onChange={e=>setSelected({...selected,position:e.target.value})}><option value="">직위를 선택하세요</option>{positions.filter(p=>p.is_active||p.name===selected.position).map(p=><option key={p.id} value={p.name}>{p.name}{!p.is_active?' (비활성)':''}</option>)}</select></label><label>재직 상태<select value={selected.employment_status} onChange={e=>setSelected({...selected,employment_status:e.target.value})}><option>재직</option><option>휴직</option><option>퇴직</option></select></label><label>입사일<input type="date" value={selected.joined_date||''} onChange={e=>setSelected({...selected,joined_date:e.target.value})}/></label>{canAssign&&<label>담당 팀장<select required value={selected.manager_id||''} onChange={e=>setSelected({...selected,manager_id:e.target.value})}><option value="">선택</option>{accounts.filter(a=>a.account_status==='active'&&['manager','facility_manager','admin'].includes(a.role)&&(a.role==='admin'||String(a.facility_id)===String(selected.facility_id))).map(a=><option key={a.id} value={a.id}>{a.name||a.email} · {a.department||roleLabel[a.role]}</option>)}</select><small className="field-help">선택한 부서의 활성 팀장만 표시됩니다.</small></label>}<label className="span2">메모<textarea value={selected.memo||''} onChange={e=>setSelected({...selected,memo:e.target.value})}/></label><FormActions onCancel={()=>setSelected(null)}/></form></Modal>}
...
V4.9 Supabase SQL을 먼저 실행했는지 확인하세요.`);return}
  ...
  async function exportEmployees(){downloadCsv('직원목록.csv',employees.map(e=>({직원명:e.name||'',시설:facilityName(e.facility_id),부서:e.department||'',직위:e.position||'',재직상태:e.employment_status||'',입사일:e.joined_date||'',담당팀장명:managerName(e.manager_id),메모:e.memo||''})))}
  async function exportGoals(){const {data,error}=await supabase.from('goals').select('*').order('created_at');if(error){alert(error.message);return}downloadCsv('성과목표.csv',(data||[]).map(g=>{const emp=employees.find(e=>e.id===g.owner_id);return {목표명:g.title||'',직원명:emp?.name||'',팀장명:managerName(emp?.manager_id),시설:facilityName(emp?.facility_id),부서:emp?.department||'',기간:g.period||'',분류:g.category||'',상태:g.status||'',진행률:g.progress||0,마감일:g.due_date||'',목표값:g.target_value||'',현재값:g.current_value||'',성과근거:g.evidence||'',관리자피드백:g.manager_feedback||''}}))}
  async function exportAllInterviews(){const {data,error}=await supabase.from('interviews').select('*').order('interview_date');if(error){alert(error.message);return}downloadCsv('면담기록.csv',(data||[]).map(r=>{const emp=employees.find(e=>e.id===r.employee_id);return {직원명:emp?.name||'',팀장명:managerName(r.manager_id||emp?.manager_id),시설:facilityName(emp?.facility_id),부서:emp?.department||'',면담일:r.interview_date||'',면담유형:r.interview_type||'',상태:r.mood||'',핵심요약:r.summary||'',강점:r.strengths||'',우려사항:r.concerns||'',실행항목:r.action_items||'',직원약속:r.employee_commitment||'',팀장지원:r.manager_support||'',다음면담일:r.next_date||'',공개범위:r.visibility||''}}))}
  function exportAccounts(){downloadCsv('팀장계정.csv',accounts.map(a=>({이름:a.name||'',이메일:a.email||'',시설:facilityName(a.facility_id),부서:a.department||'',직위:a.position||'',권한:roleLabel[a.role]||a.role||'',계정상태:a.account_status==='active'?'활성':a.account_status==='pending'?'승인대기':'중지',가입일:a.created_at?new Date(a.created_at).toLocaleString('ko-KR'):''})))}
  ...
  return <>
    ...
    {tab==='accounts'&&<>
      ...
      <Panel title="전체 팀장 계정"><div className="table-wrap"><table><thead><tr><th>이름</th><th>이메일</th><th>시설</th><th>부서</th><th>권한</th><th>상태</th><th/></tr></thead><tbody>{accounts.map(a=><tr key={a.id}><td><strong>{a.name||'-'}</strong></td><td>{a.email}</td><td>{facilityName(a.facility_id)}</td><td>{a.department||'-'}</td><td>{roleLabel[a.role]}</td><td><Badge status={a.account_status==='active'?'활성':a.account_status==='pending'?'승인대기':'중지'}/></td><td><div className="account-row-actions"><button className="icon-btn" onClick={()=>setAccountEdit({...a})} title="계정 수정"><Pencil size={16}/></button><button className="icon-btn danger-text" onClick={()=>deleteAccount(a)} title="계정 삭제" disabled={busy||a.id===profile?.id}><Trash2 size={16}/></button></div></td></tr>)}</tbody></table></div></Panel>
    </>}
