...
function AuthScreen(){
  const [mode,setMode]=useState('login'),[email,setEmail]=useState(()=>localStorage.getItem('rememberedLoginEmail')||''),[password,setPassword]=useState(''),[name,setName]=useState(''),[msg,setMsg]=useState(''),[busy,setBusy]=useState(false),[showPassword,setShowPassword]=useState(false),[rememberEmail,setRememberEmail]=useState(()=>Boolean(localStorage.getItem('rememberedLoginEmail')))
  async function submit(e){
    ...
    try{
      ...
      if(r.error){
        ...
        setPassword('')
        setMsg('가입 요청이 접수되었습니다. 관리자가 승인하면 로그인하여 사용할 수 있습니다.')
      }else if(mode==='login'){
  ...
  return <div className="auth-wrap">
    ...
    <div className="auth-form-side"><form className="auth-card" onSubmit={submit}>
      ...
      <button type="button" className="text-btn" onClick={()=>{setMode(mode==='login'?'signup':'login');setMsg('')}}>{mode==='login'?'팀장 계정이 없나요? 가입 요청':'이미 가입 요청을 했나요? 로그인'}</button>
    </form></div>
  </div>
}
...
function Interviews({profile}){
  ...
  useEffect(()=>{load()},[])
  async function load(){const [a,b]=await Promise.all([supabase.from('interviews').select('*').order('interview_date',{ascending:false}),supabase.from('employees').select('id,name,department,employment_status')]);setRows(a.data||[]);setPeople((b.data||[]).filter(x=>x.employment_status!=='퇴직'));sessionStorage.removeItem('dashboardTargetEmployee')}
...
function AdminPage(){
  const [tab,setTab]=useState('alerts'),[settings,setSettings]=useState(DEFAULT_SETTINGS),[accounts,setAccounts]=useState([]),[employees,setEmployees]=useState([]),[departments,setDepartments]=useState([]),[busy,setBusy]=useState(false),[notice,setNotice]=useState(''),[accountEdit,setAccountEdit]=useState(null),[newDepartment,setNewDepartment]=useState('')
  useEffect(()=>{load()},[])
  async function load(){
    ...
    if(s.data)setSettings({...DEFAULT_SETTINGS,...s.data});setAccounts(a.data||[]);setEmployees(e.data||[]);setDepartments(d.data||[])
  }
  ...
  async function removeDepartment(row){if(!confirm(`${row.name} 부서를 목록에서 삭제할까요? 기존 직원 정보는 그대로 유지됩니다.`))return;const r=await supabase.from('org_departments').delete().eq('id',row.id);if(r.error)alert(r.error.message);else load()}
  ...
  return <>
      ...
      <Panel title={`신규 가입 요청 ${pending}건`} action={<span className="panel-caption">승인 전에는 로그인할 수 없습니다.</span>}>
        <div className="approval-request-list">
          {accounts.filter(a=>a.account_status==='pending').length?accounts.filter(a=>a.account_status==='pending').map(a=><div className="approval-request-card" key={a.id}>
            ...
            <div className="approval-request-actions"><button className="secondary" onClick={()=>setAccountEdit({...a})}><Pencil size={15}/> 상세</button><button className="secondary danger-text" onClick={()=>rejectAccount(a)}><UserX size={15}/> 반려</button><button className="primary" onClick={()=>approveAccount(a)}><UserCheck size={15}/> 승인</button></div>
          </div>):<div className="empty compact success-empty"><CheckCircle2/> 대기 중인 가입 요청이 없습니다.</div>}
        </div>
      </Panel>
      <Panel title="전체 팀장 계정"><div className="table-wrap"><table><thead><tr><th>이름</th><th>이메일</th><th>부서</th><th>권한</th><th>상태</th><th/></tr></thead><tbody>{accounts.map(a=><tr key={a.id}><td><strong>{a.name||'-'}</strong></td><td>{a.email}</td><td>{a.department||'-'}</td><td>{roleLabel[a.role]}</td><td><Badge status={a.account_status==='active'?'활성':a.account_status==='pending'?'승인대기':'중지'}/></td><td><button className="icon-btn" onClick={()=>setAccountEdit({...a})}><Pencil size={16}/></button></td></tr>)}</tbody></table></div></Panel>
    </>}
    ...
    {tab==='master'&&<div className="admin-grid"><Panel title="부서 목록"><form className="inline-add" onSubmit={addDepartment}><input value={newDepartment} onChange={e=>setNewDepartment(e.target.value)} placeholder="새 부서명"/><button className="primary"><Plus size={16}/> 추가</button></form><div className="master-list">{departments.map(d=><div key={d.id}><span>{d.name}</span><button className="icon-btn danger-text" onClick={()=>removeDepartment(d)}><Trash2 size={15}/></button></div>)}{!departments.length&&<div className="empty compact">등록된 부서가 없습니다.</div>}</div></Panel><Panel title="운영 안내"><div className="guide-box"><strong>기초정보 사용 방법</strong><p>부서 목록은 관리자 기준정보입니다. 직원·팀장 등록 화면에서 동일한 명칭을 사용하면 검색과 집계가 정확해집니다.</p><p>기존 직원의 부서명은 목록을 삭제해도 자동 변경되지 않습니다.</p></div></Panel></div>}
    {tab==='data'&&<div className="admin-grid"><Panel title="데이터 다운로드"><div className="export-list"><button onClick={()=>downloadCsv('직원목록.csv',employees)}><Users size={19}/><div><strong>직원 목록</strong><small>{employees.length}명 · 담당 팀장 포함</small></div><Download size={17}/></button><button onClick={async()=>{const {data}=await supabase.from('goals').select('*');downloadCsv('성과목표.csv',data||[])}}><Target size={19}/><div><strong>성과 목표</strong><small>전체 목표와 진행률</small></div><Download size={17}/></button><button onClick={async()=>{const {data}=await supabase.from('interviews').select('*');downloadCsv('면담기록.csv',data||[])}}><MessageSquareText size={19}/><div><strong>면담 기록</strong><small>전체 면담과 후속 실행</small></div><Download size={17}/></button><button onClick={()=>downloadCsv('팀장계정.csv',accounts.map(({id,name,email,department,position,role,account_status,created_at})=>({id,name,email,department,position,role,account_status,created_at})))}><ShieldCheck size={19}/><div><strong>팀장 계정</strong><small>권한과 승인 상태</small></div><Download size={17}/></button></div></Panel><Panel title="데이터 보호"><div className="guide-box"><strong>안전한 운영 원칙</strong><p>다운로드한 파일에는 인사정보와 면담 내용이 포함될 수 있습니다. 업무용 저장소에서만 보관하고 불필요한 사본은 삭제하세요.</p><p>이 화면에서는 일괄 삭제 기능을 제공하지 않아 실수로 데이터가 사라지는 것을 방지합니다.</p></div></Panel></div>}
    {accountEdit&&<Modal title="팀장 계정 승인·권한" onClose={()=>setAccountEdit(null)}><form className="form-grid" onSubmit={saveAccount}><label>이름<input value={accountEdit.name||''} onChange={e=>setAccountEdit({...accountEdit,name:e.target.value})}/></label><label>이메일<input disabled value={accountEdit.email||''}/></label><label>부서<input list="department-options" value={accountEdit.department||''} onChange={e=>setAccountEdit({...accountEdit,department:e.target.value})}/></label><label>직위<input value={accountEdit.position||''} onChange={e=>setAccountEdit({...accountEdit,position:e.target.value})}/></label><label>권한<select value={accountEdit.role} onChange={e=>setAccountEdit({...accountEdit,role:e.target.value})}><option value="manager">팀장</option><option value="admin">관리자</option></select></label><label>계정 상태<select value={accountEdit.account_status} onChange={e=>setAccountEdit({...accountEdit,account_status:e.target.value})}><option value="pending">승인대기</option><option value="active">활성</option><option value="inactive">중지</option></select></label><FormActions onCancel={()=>setAccountEdit(null)}/></form></Modal>}
    <datalist id="department-options">{departments.map(d=><option key={d.id} value={d.name}/>)}</datalist>
  </>
}
