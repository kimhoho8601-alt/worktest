...
function Employees({profile}){
  ...
  return <>
    ...
    {isAdmin&&<Panel title="팀장 계정 승인·관리"><div className="table-wrap"><table><thead><tr><th>이름</th><th>이메일</th><th>권한</th><th>상태</th><th>소속 시설</th><th/></tr></thead><tbody>{accounts.map(a=><tr key={a.id}><td>{a.name||'-'}</td><td>{a.email}</td><td>{roleLabel[a.role]}</td><td><Badge status={a.account_status==='active'?'활성':a.account_status==='pending'?'승인대기':'중지'}/></td><td>{a.department||'-'}</td><td><button className="icon-btn" onClick={()=>setAccountEdit({...a})}><Pencil size={16}/></button></td></tr>)}</tbody></table></div></Panel>}
