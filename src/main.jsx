...
function AdminPage({profile}){
  const [tab,setTab]=useState('alerts'),[settings,setSettings]=useState(DEFAULT_SETTINGS),[accounts,setAccounts]=useState([]),[employees,setEmployees]=useState([]),[facilities,setFacilities]=useState([]),[departments,setDepartments]=useState([]),[positions,setPositions]=useState([]),[menuRows,setMenuRows]=useState([]),[busy,setBusy]=useState(false),[notice,setNotice]=useState(''),[accountEdit,setAccountEdit]=useState(null),[newFacility,setNewFacility]=useState(''),[newDepartment,setNewDepartment]=useState(''),[newPosition,setNewPosition]=useState(''),[departmentFacility,setDepartmentFacility]=useState('')
  useEffect(()=>{load()},[])
  async function load(){
