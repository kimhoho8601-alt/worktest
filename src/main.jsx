...
function AdminPage(){
  ...
  async function toggleDepartment(row){const next=!row.is_active;const r=await supabase.from('org_departments').update({is_active:next}).eq('id',row.id);if(r.error)alert(r.error.message);else{setNotice(`${row.name} 부서를 ${next?'활성화':'비활성화'}했습니다.`);load()}}
  ...
  return <>
    ...
    {tab==='master'&&<div className="admin-grid"><Panel title="부서 목록"><form className="inline-add" onSubmit={addDepartment}><input value={newDepartment} onChange={e=>setNewDepartment(e.target.value)} placeholder="새 부서명"/><button className="primary"><Plus size={16}/> 추가</button></form><div className="master-list">{departments.map(d=><div key={d.id} className={!d.is_active?'master-inactive':''}><span><strong>{d.name}</strong><small>{d.is_active?'가입 화면에 표시':'가입 화면에서 숨김'}</small></span><button className={d.is_active?'secondary compact-button':'primary compact-button'} onClick={()=>toggleDepartment(d)}>{d.is_active?'비활성화':'활성화'}</button></div>)}{!departments.length&&<div className="empty compact">등록된 부서가 없습니다.</div>}</div></Panel><Panel title="운영 안내"><div className="guide-box"><strong>기초정보 사용 방법</strong><p>활성 부서만 팀장 가입 화면의 선택 목록에 표시됩니다.</p><p>기존 기록 보호를 위해 부서는 삭제하지 않고 비활성화합니다. 승인 화면에서는 신청자가 선택한 부서를 확인하거나 변경할 수 있습니다.</p></div></Panel></div>}
