...
function AuthScreen(){
  ...
  return <div className="auth-wrap">
    <div className="auth-visual">
      ...
      <div className="auth-copy"><div className="auth-logo">SC</div><h1>직원 성과/면담을<br/><em>한곳에서 관리하세요.</em></h1><p>목표, 면담, 팀원 정보를 쉽고 직관적으로 연결합니다.</p></div>
...
function Dashboard({profile}){
  ...
  return <>
    <div className="dashboard-hero"><div><span className="eyebrow">PERFORMANCE OVERVIEW</span><h2>안녕하세요, {profile.name||'관리자'}님 👋</h2><p>{profile.role==='admin'?'전체 팀의 성장 흐름과 실행 현황을 확인하세요.':'오늘도 팀의 성장을 함께 만들어가세요.'}</p></div><div className="date-chip"><CalendarDays size={17}/>{new Date().toLocaleDateString('ko-KR',{month:'long',day:'numeric',weekday:'short'})}</div></div>
