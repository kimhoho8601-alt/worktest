...
function App(){
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobile?'open':''}`}>
      <div className="brand"><span className="brand-mark">S</span><div><strong>성과·면담</strong><small>관리 시스템 V4</small></div></div>
...
function Dashboard({profile}){
  ...
  const activePeople=people.filter(p=>p.account_status==='active')
  const noInterview=activePeople.filter(p=>!interviews.some(i=>i.employee_id===p.id&&new Date(i.interview_date)>=ninety))
  return <>
    <div className="stat-grid"><Stat icon={Target} label="관리 목표" value={goals.length} sub="현재 조회 범위"/><Stat icon={CircleGauge} label="평균 진행률" value={`${avg}%`} sub="전체 목표 평균"/><Stat icon={MessageSquareText} label="면담 기록" value={interviews.length} sub="누적 조회 건수"/><Stat icon={AlertTriangle} label="주의 항목" value={overdue.length+staleGoals.length} sub="기한 경과·장기 미갱신" danger={overdue.length+staleGoals.length>0}/></div>
    ...
    ]}/></Panel><Panel title="운영 현황"><div className="progress-ring-wrap"><div className="progress-ring" style={{'--p':`${avg*3.6}deg`}}><span>{avg}%</span></div><div><h3>조직 평균 진행률</h3><p>정기 갱신과 면담 기록을 기준으로 확인합니다.</p></div></div><div className="mini-metrics"><div><strong>{activePeople.length}</strong><span>활성 직원</span></div><div><strong>{goals.filter(g=>g.status==='완료').length}</strong><span>완료 목표</span></div><div><strong>{dueSoon.length}</strong><span>7일 내 마감</span></div></div></Panel></div>
...
function Goals({profile}){
  ...
  const canManage=['manager','admin'].includes(profile.role)
  useEffect(()=>{load()},[])
  async function load(){const [a,b]=await Promise.all([supabase.from('goals').select('*').order('created_at',{ascending:false}),supabase.from('profiles').select('id,name,department,account_status')]);setRows(a.data||[]);setPeople((b.data||[]).filter(x=>x.account_status==='active'))}
  function open(row){setEditId(row?.id||null);setForm(row?{...row}:{...goalBlank,owner_id:profile.id});setModal(true)}
...
function Interviews({profile}){
  ...
  useEffect(()=>{load()},[])
  async function load(){const [a,b]=await Promise.all([supabase.from('interviews').select('*').order('interview_date',{ascending:false}),supabase.from('profiles').select('id,name,department,account_status')]);setRows(a.data||[]);setPeople((b.data||[]).filter(x=>x.account_status==='active'))}
...
function Employees({profile}){
  ...
  useEffect(()=>{load()},[])
  async function load(){const r=await supabase.from('profiles').select('*').order('created_at',{ascending:false});setRows(r.data||[])}
  async function save(e){e.preventDefault();const {id,...payload}=selected;const r=await supabase.from('profiles').update(payload).eq('id',id);if(r.error)alert(r.error.message);else{setSelected(null);load()}}
  const filtered=rows.filter(x=>`${x.name} ${x.email} ${x.department}`.toLowerCase().includes(search.toLowerCase()))
  return <><div className="toolbar"><div className="search"><Search size={17}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="직원 검색"/></div><div className="summary-chip"><Users size={16}/> 전체 {filtered.length}명 · 승인대기 {filtered.filter(x=>x.account_status==='pending').length}명</div></div><div className="employee-grid">{filtered.map(x=><div className="employee-card" key={x.id}><div className="avatar large">{(x.name||x.email||'?')[0]}</div><div className="grow"><h3>{x.name||'이름 미입력'} <Badge status={x.account_status==='active'?'활성':x.account_status==='pending'?'승인대기':'중지'}/></h3><p>{x.department||'부서 미지정'} · {x.position||'직위 미지정'}</p><small>{x.email} · {roleLabel[x.role]}</small></div>{isAdmin&&<button className="icon-btn" onClick={()=>setSelected({...x})}><Pencil size={16}/></button>}</div>)}</div>
  {selected&&<Modal title="직원 정보·권한 수정" onClose={()=>setSelected(null)}><form className="form-grid" onSubmit={save}><label>이름<input value={selected.name||''} onChange={e=>setSelected({...selected,name:e.target.value})}/></label><label>이메일<input disabled value={selected.email||''}/></label><label>부서<input value={selected.department||''} onChange={e=>setSelected({...selected,department:e.target.value})}/></label><label>직위<input value={selected.position||''} onChange={e=>setSelected({...selected,position:e.target.value})}/></label><label>권한<select value={selected.role} onChange={e=>setSelected({...selected,role:e.target.value})}><option value="employee">직원</option><option value="manager">팀장</option><option value="admin">관리자</option></select></label><label>계정 상태<select value={selected.account_status} onChange={e=>setSelected({...selected,account_status:e.target.value})}><option value="pending">승인대기</option><option value="active">활성</option><option value="inactive">중지</option></select></label><label>재직 상태<select value={selected.employment_status} onChange={e=>setSelected({...selected,employment_status:e.target.value})}><option>재직</option><option>휴직</option><option>퇴직</option></select></label><label>입사일<input type="date" value={selected.joined_date||''} onChange={e=>setSelected({...selected,joined_date:e.target.value})}/></label><label className="span2">직속 관리자<select value={selected.manager_id||''} onChange={e=>setSelected({...selected,manager_id:e.target.value||null})}><option value="">없음</option>{rows.filter(r=>['manager','admin'].includes(r.role)&&r.id!==selected.id).map(r=><option key={r.id} value={r.id}>{r.name} · {roleLabel[r.role]}</option>)}</select></label><FormActions onCancel={()=>setSelected(null)}/></form></Modal>}</>
}
