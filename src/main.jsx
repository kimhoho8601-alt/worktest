...
function AuthScreen(){
  ...
  return <div className="auth-wrap">
    <div className="auth-visual">
      ...
      <div className="auth-copy"><img className="auth-brand-logo" src="/Logo-white.png" alt="Save the Children"/><h1>직원 성장의 과정을<br/><em>한곳에서 관리하세요.</em></h1><p>목표, 면담, 팀원 정보를 쉽고 직관적으로 연결합니다.</p></div>
    ...
    <div className="auth-form-side"><form className="auth-card" onSubmit={submit}>
      ...
      {mode==='signup'&&<><label>이름<input required value={name} onChange={e=>setName(e.target.value)} placeholder="세이브"/></label><label>소속 시설<select required value={department} onChange={e=>setDepartment(e.target.value)}><option value="">부서를 선택하세요</option>{departments.map(d=><option key={d.id} value={d.name}>{d.name}</option>)}</select><small className="field-help">목록에 부서가 없다면 관리자에게 등록을 요청하세요.</small></label></>}
