  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="content">
        {page==='dashboard'&&<Dashboard profile={profile} onNavigate={setPage}/>} {page==='goals'&&<Goals profile={profile}/>} {page==='interviews'&&<Interviews profile={profile}/>} {page==='employees'&&canManage&&<Employees profile={profile}/>} 
      </section>
    </main>
  </div>
}
...
function Dashboard({profile,onNavigate}){
  ...
  const alerts=[
    ...pendingAccounts.map(x=>({level:'approval',title:`팀장 승인 대기: ${x.name||x.email}`,sub:'계정 승인 후 시스템 사용 가능',action:'직원관리',page:'employees'})),
    ...overdue.map(x=>({level:'danger',title:`기한 경과: ${x.title}`,sub:`마감 ${x.due_date}`,action:'성과관리',page:'goals'})),
    ...dueSoon.map(x=>({level:'warn',title:`7일 이내 마감: ${x.title}`,sub:`마감 ${x.due_date}`,action:'성과관리',page:'goals'})),
    ...staleGoals.map(x=>({level:'warn',title:`30일 이상 미갱신: ${x.title}`,sub:'진행률 점검 필요',action:'성과관리',page:'goals'})),
    ...noGoal.map(x=>({level:'info',title:`목표 미등록: ${x.name}`,sub:x.department||'부서 미지정',action:'성과관리',page:'goals'})),
    ...noInterview.map(x=>({level:'info',title:`최근 90일 면담 없음: ${x.name}`,sub:x.department||'부서 미지정',action:'면담관리',page:'interviews',employeeId:x.id}))
  ]
  const recent=[
  ...
  return <>
    ...
    <div className="dashboard-grid">
      ...
      <Panel title="빠른 현황"><div className="quick-insights"><Insight icon={ClipboardCheck} label="목표 미등록" value={noGoal.length} tone="warn" onClick={()=>navigate('goals')}/><Insight icon={MessageSquareText} label="90일 미면담" value={noInterview.length} tone="info" onClick={()=>navigate('interviews')}/><Insight icon={Activity} label="30일 미갱신" value={staleGoals.length} tone="danger" onClick={()=>navigate('goals')}/><Insight icon={TrendingUp} label="목표 완료율" value={`${goals.length?Math.round(completed.length/goals.length*100):0}%`} tone="good" onClick={()=>navigate('goals')}/></div></Panel>
    </div>
  </>
}
