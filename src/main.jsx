    ...
    try{
      if(mode==='login'){
        ...
        if(error){
          ...
          if(message.includes('invalid login credentials')) setMsg('이메일 또는 비밀번호가 올바르지 않습니다.')
          else if(message.includes('email not confirmed')) setMsg('이메일 인증이 완료되지 않았습니다. 관리자에게 문의해 주세요.')
      ...
      if(error){
        ...
        if(message.includes('email rate limit exceeded')){
          setMsg('회원가입 확인 메일 발송 한도를 초과했습니다. Supabase에서 이메일 확인 기능을 끄거나 잠시 후 다시 시도해 주세요.')
      ...
      // 이메일 확인을 끈 경우 가입 직후 세션이 생성될 수 있으므로,
      // 관리자 승인 전 자동 로그인되지 않도록 즉시 로그아웃합니다.
      if(data?.session) await supabase.auth.signOut()

      setMsg('팀장 계정 가입이 접수되었습니다. 관리자 승인 후 로그인할 수 있습니다.')
      setPassword('')
    }finally{
      setBusy(false)
    }
