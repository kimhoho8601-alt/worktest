...
function Dashboard({profile,onNavigate}){
  ...
  return <>
    <div className="dashboard-hero"><div><span className="eyebrow">TODAY'S HR BRIEFING</span><h2>안녕하세요, {profile.name||'관리자'}님 👋</h2><p>{profile.role==='admin'?'전체 시설의 주요 변화와 오늘 처리할 업무를 확인하세요.':profile.role==='facility_manager'?'소속 시설 전체의 면담·목표 현황을 확인하세요.':'내 팀의 면담·목표 현황과 우선 업무를 확인하세요.'}</p></div><div className="date-chip"><CalendarDays size={17}/>{new Date().toLocaleDateString('ko-KR',{month:'long',day:'numeric',weekday:'short'})}</div></div>
